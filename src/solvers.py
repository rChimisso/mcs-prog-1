...
class IterativeSolver(ABC):
  ...
  """
  def __init__(self, tol: float = 1e-8, max_iter: int = 20000) -> None:
    """
    Method agnostic iterative solver for SPD matrices.
  ...
  def _solve(self, A: np.typing.NDArray[Any], b: np.typing.NDArray[Any], x0: np.typing.NDArray[Any], tol: float, max_iter: int) -> tuple[np.typing.NDArray[Any], int, bool]:
    ...
    :rtype: tuple[np.typing.NDArray[Any], int, bool]
    """
...
class JacobiSolver(IterativeSolver):
  ...
  def _solve(self, A: np.typing.NDArray[Any], b: np.typing.NDArray[Any], x0: np.typing.NDArray[Any], tol: float, max_iter: int) -> tuple[np.typing.NDArray[Any], int, bool]:
    ...
    D = np.diag(A)
    R = A - np.diagflat(D)
    for k in tqdm(range(max_iter), desc="Jacobi", unit="iter"):
      x = (b - R @ x) / D
...
class GaussSeidelSolver(IterativeSolver):
  ...
  Gauss-Seidel is guaranteed to converge for any SPD matrix eventually.
  """
  def _solve(self, A: np.typing.NDArray[Any], b: np.typing.NDArray[Any], x0: np.typing.NDArray[Any], tol: float, max_iter: int) -> tuple[np.typing.NDArray[Any], int, bool]:
    x, n = x0, b.size
    D, L, U = np.diag(A), np.tril(A, -1), np.triu(A,  1)
    for k in tqdm(range(max_iter), desc="GaussSeidel", unit="iter"):
      for i in range(n):
  ...
  *xᵏ⁺¹ = xᵏ + αᵏ rᵏ*  
  ...
  def _solve(self, A: np.typing.NDArray[Any], b: np.typing.NDArray[Any], x0: np.typing.NDArray[Any], tol: float, max_iter: int) -> tuple[np.typing.NDArray[Any], int, bool]:
    ...
    if np.linalg.norm(r) > tol * np.linalg.norm(b):
      for k in tqdm(range(max_iter), desc="GradientDescent", unit="iter"):
        Ar = A @ r
        alpha = (r @ r) / (r @ Ar)
        x = x + alpha * r
        r = r - alpha * Ar
...
class ConjugateGradientSolver(IterativeSolver):
  ...
  *pᵏ⁺¹ = rᵏ⁺¹ + βᵏ pᵏ*  
  ...
  def _solve(self, A: np.typing.NDArray[Any], b: np.typing.NDArray[Any], x0: np.typing.NDArray[Any], tol: float, max_iter: int) -> tuple[np.typing.NDArray[Any], int, bool]:
    x, r = x0, b - A @ x0
    p, rs_old = r.copy(), r @ r
    for k in tqdm(range(max_iter), desc="ConjugateGradient", unit="iter"):
      Ap = A @ p
      alpha = rs_old / (p @ Ap)
      x = x + alpha * p
      r = r - alpha * Ap
